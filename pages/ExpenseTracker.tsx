...
const ExpenseTracker: React.FC<ExpenseTrackerProps> = ({ onBack, onSave }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-background select-none relative">
      ...
      <div className="w-full mb-4 px-4 flex-shrink-0">
          <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 w-full flex-nowrap items-center">
            {currentCategories.map(cat => (
                <button
                    key={cat.id}
                    onClick={() => setSelectedCategory(cat.id)}
                    className={`flex-shrink-0 flex flex-col items-center gap-2 min-w-[72px] p-3 rounded-xl border transition-all ${selectedCategory === cat.id ? 'bg-surface border-primary scale-105' : 'bg-transparent border-transparent opacity-50'}`}
                >
